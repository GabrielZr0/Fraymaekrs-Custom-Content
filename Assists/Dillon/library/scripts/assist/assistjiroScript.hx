...
function update(){
	...
	if (self.inState(STATE_IDLE)) {
		...
		if (self.isOnFloor()) {
			...
			var proj1 = match.createProjectile(self.getResource().getContent("assistjiroProjectile"), self);
			var proj2 = match.createProjectile(self.getResource().getContent("assistjiroProjectile"), self);
