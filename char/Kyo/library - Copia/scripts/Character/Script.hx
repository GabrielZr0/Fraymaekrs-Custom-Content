...
//-----------SPECIAL UP-----------
function held_UpSpecial() {
    Common.onButtonsHeld(Buttons.ATTACK, function () {
        if (self.onButtonsHeld().ATTACK) {
            self.updateHitboxStats(0, { damage:3, knockbackGrowth: 30, baseKnockback: 60});
            self.updateAnimationStats({gravityMultiplier:0.75});
        }
    });
}
